...
class ControllerExtensionFeedTrsSitemap extends Controller {
  ...
  public function install() {
    ...
    $this->model_extension_feed_trs_sitemap->editSetting('feed_trs_sitemap', 'feed_trs_sitemap_key', substr(md5(rand() . microtime()), 0, 6));
  }
  ...
  public function index() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
      ...
      foreach ($this->request->post as $key => $value) {
        $this->model_extension_feed_trs_sitemap->editSetting('feed_trs_sitemap', $key, $value);
      }
    ...
    $data['breadcrumbs'][] = array(
      ...
      'href' => $this->url->link('extension/feed/trs_sitemap', 'user_token=' . $this->session->data['user_token'], true)
    );

    $data['action'] = $this->url->link('extension/feed/trs_sitemap', 'user_token=' . $this->session->data['user_token'], true);
    $data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=feed', true);

    if (isset($this->request->post['feed_trs_sitemap_status'])) {
      $data['feed_trs_sitemap_status'] = $this->request->post['feed_trs_sitemap_status'];
    } else {
      $data['feed_trs_sitemap_status'] = $this->config->get('feed_trs_sitemap_status');
    }

    if (isset($this->request->post['feed_trs_sitemap_limit'])) {
      $data['feed_trs_sitemap_limit'] = $this->request->post['feed_trs_sitemap_limit'];
    } else if ($this->config->get('feed_trs_sitemap_limit')) {
      $data['feed_trs_sitemap_limit'] = $this->config->get('feed_trs_sitemap_limit');
    } else {
      $data['feed_trs_sitemap_limit'] = 50000;
    }

    if (isset($this->request->post['feed_trs_sitemap_duration'])) {
      $data['feed_trs_sitemap_duration'] = $this->request->post['feed_trs_sitemap_duration'];
    } else if ($this->config->get('feed_trs_sitemap_duration')) {
      $data['feed_trs_sitemap_duration'] = $this->config->get('feed_trs_sitemap_duration');
    } else {
      $data['feed_trs_sitemap_duration'] = 24;
    }

    $data['data_crontab_task'] = sprintf('* * * * * /usr/bin/curl --silent \'%sindex.php?route=extension/feed/trs_sitemap&key=%s\' &> /dev/null', HTTPS_CATALOG, $this->config->get('feed_trs_sitemap_key'));
    $data['data_sitemap_url']  = sprintf('%ssitemap.xml', HTTPS_CATALOG);
    $data['data_duration']     = $this->estimateDuration();
    $data['data_user_token']   = $this->session->data['user_token'];
  ...
  public function estimateDuration() {
    ...
    if (isset($this->request->get['duration']) && $this->request->get['duration'] > 0) {
      ...
      $duration = $this->config->get('feed_trs_sitemap_duration');
    } else {
      $duration = 24;
    }
  ...
  protected function validate() {
    ...
    if ($this->request->post['feed_trs_sitemap_limit'] > 50000) {
      $this->_error['warning'] = $this->language->get('error_limit_max_reached');
    }

    if ($this->request->post['feed_trs_sitemap_limit'] < 100) {
      $this->_error['warning'] = $this->language->get('error_limit_min_reached');
    }

    if ($this->request->post['feed_trs_sitemap_duration'] < 1) {
